                      ...
                      {msg.role === 'assistant' ? (
                        <ReactMarkdown remarkPlugins={[remarkGfm]} components={{
                          code({node, inline, className, children, ...props}) {
                            return inline
                              ? <code className="bg-gray-700 px-1 py-0.5 rounded text-xs font-mono" {...props}>{children}</code>
                              : <pre className="bg-gray-900 rounded-lg p-3 overflow-x-auto my-2 text-xs"><code className="font-mono text-green-400" {...props}>{children}</code></pre>
                          },
                          ...
                          table: ({children}) => <div className="overflow-x-auto my-2"><table className="border-collapse border border-gray-600 text-xs w-full">{children}</table></div>,
                          ...
                          a: ({children, href}) => <a href={href} target="_blank" className="text-blue-400 underline">{children}</a>,
                        }}>
                          {msg.content || '...'}
                        </ReactMarkdown>
                      ) : (
          ...
          <div className="max-w-3xl mx-auto">
            {fileName && (
              <div className="flex items-center gap-2 bg-gray-800 rounded-lg px-3 py-2 mb-2 text-xs text-gray-300 border border-gray-700">
                ...
                <button onClick={() => { setFileContext(null); setFileName(null) }} className="text-gray-500 hover:text-red-400">
                  <X size={13} />
                </button>
              </div>
            )}
            <input type="file" ref={fileInputRef} onChange={handleFileUpload} className="hidden" accept=".txt,.csv,.docx,.pdf,.jpg,.jpeg,.png" />
